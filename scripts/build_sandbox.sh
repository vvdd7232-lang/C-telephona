...
mkdir -p qtpkg
pip3 download PySide6-Essentials --no-deps --only-binary=:all: -d qtpkg -q 2>/dev/null || \
python3 -m pip download PySide6-Essentials --no-deps --only-binary=:all: -d qtpkg -q
WHEEL="$(ls qtpkg/PySide6-Essentials-*.whl | head -1)"
python3 -m zipfile -e "$WHEEL" qt6
rm -rf qtpkg

# dev-симлинки для линковки
cd "$BUILD/qt6/PySide6/Qt/lib"
for f in libQt6Core libQt6Gui libQt6Widgets; do
...
cd qtbase
git sparse-checkout set include src/corelib src/gui src/widgets 2>/dev/null || true
...
def gen(pat, outname):
    ...
    return syms
gen(re.compile(r'^egl[A-Za-z0-9_]*$'), 'libEGL.so.1')
gen(re.compile(r'^(gl|glX)[A-Za-z0-9_]*$'), 'libGL.so.1')
gen(re.compile(r'^xkb_[A-Za-z0-9_]*$'), 'libxkbcommon.so.0')
gen(re.compile(r'^dbus_[A-Za-z0-9_]*$'), 'libdbus-1.so.3')
...
VERSION_MAPS=("libxkbcommon.so.0:V_0.5.0" "libdbus-1.so.3:LIBDBUS_1_3")
...
g++ -std=c++17 -fPIC -O2 \
    ...
    "$REPO/src/main.cpp" "$REPO/src/MainWindow.cpp" "$REPO/src/pixelart.cpp" \
    -I "$BUILD/qt6/headers" \
    -I "$BUILD/qt6/headers/QtCore" -I "$BUILD/qt6/headers/QtGui" -I "$BUILD/qt6/headers/QtWidgets" \
    -L "$BUILD/qt6/PySide6/Qt/lib" -lQt6Widgets -lQt6Gui -lQt6Core \
...
LC_ALL=C.UTF-8 \
"$BUILD/bin/enderforge" --screenshot "$REPO/docs/preview.png"
